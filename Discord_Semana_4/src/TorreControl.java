...
public class TorreControl {
	...
	//
	/**Este metodo asigna trabajadores random para que trabajen
	 * Los saca de la planilla y los asigna a otra
	 */
		...
		for (int i = 0; i < operadoresActivosHoy.length; i++) { //voy a llenar una lista de operadoresActivosHoy
			do {
				...
				if (auxComparar != auxRepetido) { 
					//TODO
					//lo del cumpleaños es un if mas para desarrollar, hoy JODETE, EL PAIS SE SACA LABURANDO LOCO
	...
	private void quienTrabajaHoy() {
		if (operadoresActivosHoy.length>0) {
			for (Operador operador : operadoresActivosHoy) {
				System.out.println(operador); //soy re capo y ya con llamar al objeto me devuelve el metodo tostring de la clase operador.
			}
		} 
	...
	public void cargarListaAeronaves() {
		...
		//la idea es cargarlos todos en FALSE que no esten habilitados a instalar hasta que un operador le asigna la pista.
		...
		//muchasNaves[6] = new Boing("XIZ-123", "Blanco", 200, 20.0d, false, "Aerolineas Argentinas", "Fede");
		//
	}
